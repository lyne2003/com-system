...
<div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
    ...
    <form method="POST" action="{{ route('rfqs.update', $rfq->id) }}">
    @csrf
        ...
        <div class="grid grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input
                    type="text"
                    name="reference"
                    value="{{ old('reference', $rfq->reference) }}"
            ...
            <div>
                ...
                <input
                    type="text"
                    name="inquiry_n"
                    value="{{ old('inquiry_n', $rfq->inquiry_n) }}"
            ...
            <div>
                ...
                <input
                    type="date"
                    name="date"
                    value="{{ old('date', $rfq->date ? \Carbon\Carbon::parse($rfq->date)->format('Y-m-d') : '') }}"
                    class="w-full border rounded p-2">
            </div>
            ...
            <div>
                ...
                <select name="priority" class="w-full border rounded p-2">
                    ...
                    <option value="Low" @if(old('priority', $rfq->priority)=='Low') selected @endif>Low</option>
                    <option value="Medium" @if(old('priority', $rfq->priority)=='Medium') selected @endif>Medium</option>
                    <option value="High" @if(old('priority', $rfq->priority)=='High') selected @endif>High</option>
                    <option value="Urgent" @if(old('priority', $rfq->priority)=='Urgent') selected @endif>Urgent</option>
                </select>
            </div>
        ...
        <div class="grid grid-cols-2 gap-6 mt-6">
            ...
            <div>
                ...
                <textarea
                    ...
                    rows="3"
                    class="w-full border rounded p-2"
                    placeholder="Internal notes for purchasing team...">{{ old('notes_to_purchasing', $rfq->notes_to_purchasing) }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea
                    ...
                    rows="3"
                    class="w-full border rounded p-2"
                    placeholder="Notes for Elias...">{{ old('notes_to_elias', $rfq->notes_to_elias) }}</textarea>
            </div>
    ...
    <div class="bg-white shadow rounded-lg p-6 mb-6">
        ...
        <table class="w-full border text-sm">
            ...
            <tbody id="itemsTable">
                ...
                <tr class="item-row">
                    <td><input name="items[{{ $i }}][line_number]" value="{{ $item->line_number }}" class="w-full border p-1 text-center" readonly></td>
                    <td><input name="items[{{ $i }}][overallcode]" value="{{ $item->overallcode }}" class="w-full border p-1"></td>
                    <td><input name="items[{{ $i }}][partnumber]" value="{{ $item->partnumber }}" class="w-full border p-1" placeholder="Required"></td>
                    <td><input name="items[{{ $i }}][qty]" type="number" value="{{ $item->qty }}" class="w-full border p-1"></td>
                    <td><input name="items[{{ $i }}][uom]" value="{{ $item->uom }}" class="w-full border p-1" placeholder="pcs"></td>
                    <td><input name="items[{{ $i }}][target_price]" type="number" step="0.01" value="{{ $item->target_price }}" class="w-full border p-1"></td>
                    <td>
                        <select name="items[{{ $i }}][manufacturer_id]" class="w-full border p-1">
                            <option value="">-- Select --</option>
                            @foreach($manufacturers as $mfr)
                            <option value="{{ $mfr->id }}" @if($item->manufacturer_id == $mfr->id) selected @endif>
                                {{ $mfr->name }}
                            </option>
                            @endforeach
                        </select>
